...
export function CareersModal({ isOpen, onClose }: CareersModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      submissionData.append("name", formData.name);
      submissionData.append("email", formData.email);
      submissionData.append("mobile", formData.mobile); // APPEND MOBILE DATA
      submissionData.append("category", formData.category);
      submissionData.append("role", formData.role);
      ...
      if (response.ok && result.success) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            setSubmitStatus('idle');
            setFormData({ name: '', email: '', mobile: '', category: '', role: '' });
            setFile(null);
          }, 500);
        }, 3000);
      } else {
                ...
                {submitStatus === 'success' ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      <div>
                        <label className="block text-sm font-bold text-[#1B365D] mb-2">Mobile Number *</label>
                        <input
                    ...
                    {/* ROW 3: SPECIFIC ROLE (Conditional) */}
                    <AnimatePresence mode="popLayout">
                      {formData.category && (
                        <motion.div
                          initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
